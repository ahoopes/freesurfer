...
::Rasterize( const AtlasMesh* mesh )
{
  ...
  ThreadAccumDataType  t_priorCost = 0.0;
  if ( !m_IgnoreDeformationPrior )
    {
    for ( int threadNumber = 0; threadNumber < this->GetNumberOfThreads(); threadNumber++ )
      {
      // Cost
      if ( std::isnan( double(m_ThreadSpecificPriorCosts[ threadNumber ]) ) || 
          std::isinf( double(m_ThreadSpecificPriorCosts[ threadNumber ]) ) )
        {
        // Something has gone wrong
        m_MinLogLikelihoodTimesPrior = itk::NumericTraits< double >::max();
        return;
        }
      t_priorCost += m_ThreadSpecificPriorCosts[ threadNumber ];
      } // End loop over threads

    for ( int threadNumber = 1; threadNumber < this->GetNumberOfThreads(); threadNumber++ )
      {
      // Gradient
      AtlasPositionGradientThreadAccumContainerType::ConstIterator sourceIt =  m_ThreadSpecificPriorGradients[ threadNumber ]->Begin();
      AtlasPositionGradientThreadAccumContainerType::Iterator targetIt =  m_ThreadSpecificPriorGradients[ 0 ]->Begin();
      for ( ; targetIt != m_ThreadSpecificPriorGradients[ 0 ]->End(); ++targetIt, ++sourceIt )
        {
        targetIt.Value() += sourceIt.Value();
        }
      }

    AtlasPositionGradientThreadAccumContainerType::ConstIterator priorGradientIt =  m_ThreadSpecificPriorGradients[ 0 ]->Begin();
    AtlasPositionGradientContainerType::Iterator gradientIt = m_PositionGradient->Begin();
    for ( ; gradientIt != m_PositionGradient->End(); ++gradientIt, ++priorGradientIt )
      {
...
#if SUBTRACT_MARGINAL_ENTROPY 
      // ANDREW: THESE SHOULD PROBABLY BE CONVERTED
