...
MRI_SEGMENTATION *MRImaxsegment(MRI *mri, float low_val, float hi_val)
{
  ...
  */
  ...
  if (Gdiag & DIAG_WRITE && DIAG_VERBOSE_ON) MRIwrite(mri_labeled, "labeled.mgh");
  MRIfree(&mri_labeled);
...
static int mriSegmentMerge(MRI_SEGMENTATION *mriseg, int s0, int s1, MRI *mri_labeled)
{
  ...
  if (DIAG_VERBOSE_ON) fprintf(stdout, "merging segments %d and %d\n", s0, s1);
...
int MRIcompactSegments(MRI_SEGMENTATION *mriseg)
{
  ...
  if (DIAG_VERBOSE_ON) fprintf(stdout, "compacting segments...\n");
  ...
  if (newseg) {
    ...
    if (DIAG_VERBOSE_ON) fprintf(stdout, "\n        segments reduced to %d\n", s);
  }
  else {
